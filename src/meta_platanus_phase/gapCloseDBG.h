...
private:
    ...
    unsigned long long crushBubble(Int2Type<KMER31>)  {return kmer31Graph.crushBubble(); }
    unsigned long long crushBubble(Int2Type<KMER63>)  {return kmer63Graph.crushBubble(); }
    unsigned long long crushBubble(Int2Type<KMER95>)  {return kmer95Graph.crushBubble(); }
    unsigned long long crushBubble(Int2Type<KMER127>) {return kmer127Graph.crushBubble(); }
    unsigned long long crushBubble(Int2Type<KMER159>) {return kmer159Graph.crushBubble(); }
    unsigned long long crushBubble(Int2Type<KMERN>)   {return kmerNGraph.crushBubble(); }
    ...
    void save(FILE *contigFP, Int2Type<KMER31>) const { kmer31Graph.saveContigSimple(contigFP); }
    void save(FILE *contigFP, Int2Type<KMER63>) const { kmer63Graph.saveContigSimple(contigFP); }
    void save(FILE *contigFP, Int2Type<KMER95>) const { kmer95Graph.saveContigSimple(contigFP); }
    void save(FILE *contigFP, Int2Type<KMER127>) const { kmer127Graph.saveContigSimple(contigFP); }
    void save(FILE *contigFP, Int2Type<KMER159>) const { kmer159Graph.saveContigSimple(contigFP); }
    void save(FILE *contigFP, Int2Type<KMERN>) const { kmerNGraph.saveContigSimple(contigFP); }
...
public:
    ...
    : kmer31Graph(), kmer63Graph(), kmer95Graph(), kmer127Graph(), kmer159Graph(), kmerNGraph(), kmer31Counter(), kmer63Counter(), kmer95Counter(), kmer127Counter(), kmer159Counter(), kmerNCounter(), minKmer(min), maxKmer(max), minCoverage(cov), minOverlap(overlap), maxMissRate(miss), gapInfo(gap), seqLength(0), seq()
    {
...
template <unsigned M, unsigned N>
const double GapCloseDBG<M, N>::BUBBLE_THRESHOLD = 0;
...
bool GapCloseDBG<M, N>::closedGap(BruijnGraph<KMER> &graph)
{
    ...
    const auto table = graph.moveStraightTable();
    for (auto it = table.begin(), end = table.end(); it != end; ++it) {
        ...
        if ((right.*missRate * right.*overlapLength + left.*missRate * left.*overlapLength) / (right.*overlapLength + left.*overlapLength) < minRate) {
            maxLeftOverlap = left.*overlapLength;
            maxRightOverlap = right.*overlapLength;
            minRate = (right.*missRate * right.*overlapLength + left.*missRate * left.*overlapLength) / (right.*overlapLength + left.*overlapLength);
