...
void ReadFastaSingleTaggedMT(vector<SeqLib> &lib, const std::string &filename, const int numThread, const bool isMate, const bool isFastq, const bool notPair, std::unordered_map<std::string, int> &tagStringConverter)
{
    ...
    void (* const ReadFunction)(platanus::SEQ &seq, int &tagID, std::ifstream &ifs, std::unordered_map<std::string, int> &tagStringConverter) = isFastq ? ReadFastqSeqTagged : ReadFastaSeqTagged;
    for (i = 0; i < numThread; ++i)
        fseek(lib[i].pairFP, 0, SEEK_END);
    if (!forwardFile.good()) {
        throw platanus::IOError();
...
void ReadFastaPairTaggedMT(vector<SeqLib> &lib, const std::string &filename1, const std::string &filename2, const int numThread, const bool isMate, const bool isFastq, std::unordered_map<std::string, int> &tagStringConverter)
{
    ...
    platanus::SEQ forwardSeq, reverseSeq;
	int forwardTagID, reverseTagID;
    void (*ReadFunction)(platanus::SEQ &seq, int &tagID, ifstream &ifs, std::unordered_map<std::string, int> &tagStringConverter) = isFastq ? ReadFastqSeqTagged : ReadFastaSeqTagged;
    for (i = 0; i < numThread; ++i)
        fseek(lib[i].pairFP, 0, SEEK_END);
    if (!forwardFile.good() || !reverseFile.good()) {
        throw platanus::IOError();
...
void ReadFastaSeqTagged(platanus::SEQ &seq, int &tagID, std::ifstream &ifs, std::unordered_map<std::string, int> &tagStringConverter)
{
    std::string line;
...
void ReadFastqSeqTagged(platanus::SEQ &seq, int &tagID, std::ifstream &ifs, std::unordered_map<std::string, int> &tagStringConverter)
{
    unsigned readLine = 0;
    std::string line;
