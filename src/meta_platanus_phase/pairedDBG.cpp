...
//				if (node[id2Index(forwardResult.id)].length < cutoffLength) continue;
	...
	//				if (node[(-1 * reverseResult.id) - 1].length < cutoffLength || abs(forwardResult.id) == abs(reverseResult.id)) continue;
				...
				if (-graphLink.gap > std::min(tolerence, std::min(node[id2Index(graphLink.id1)].length, node[id2Index(graphLink.id2)].length)) + this->getScaffoldOverlap(graphLink.id1, graphLink.id2)) continue;
...
void PairedDBG::loadResultSeq(const long minSeqLength, const unsigned long long readLength)
{
...
void PairedDBG::makeScaffold(void)
{
    ...
    for (long i = 0; i < numNode; ++i) {
        ...
        for (long j = 0; j < node[i].numEdge; ++j) {
			...
			if (currentClusterID != node[tmpNodeID].clusterID && currentClusterID * node[tmpNodeID].clusterID != 0)
				continue;
...
//					maxCoverage = calcNodeCoverage(node[id2Index(candidate[j].id)]);
            ...
            if ((node[tmpNodeID].state & SC_INC) && (!(node[tmpNodeID].state & SC_REP)
				|| (node[tmpNodeID].state & (DBG_PRIMARY_BUBBLE | DBG_SECONDARY_BUBBLE))
				|| (currentClusterID != node[tmpNodeID].clusterID && currentClusterID * node[tmpNodeID].clusterID != 0))) {
