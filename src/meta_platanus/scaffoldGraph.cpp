...
, tolerence(0) , minTolerenceFactor(0), numContig(0), numNode(0), minTagReadToLoad(1)
, averageCoverage(0), bubbleThreshold(0)
, contigFP(NULL), bubbleFP(NULL), overlapFP(NULL), graphLinkFP(0)
, contig() , allLibraryMT(NULL), longReadLibraryMT(NULL), tagLibraryMT(NULL), targetLibraryIndex(0), coverage(), seqPool(), node()
...
void ScaffoldGraph::countMappedTagForEachContig(long numThread)
{
	...
	for (long threadID = 0; threadID < numThread; ++threadID) {
		while (fread(&forwardPosition, sizeof(platanus::Position), 1, (*tagLibraryMT)[threadID].mappedFP)) {
			fread(&reversePosition, sizeof(platanus::Position), 1, (*tagLibraryMT)[threadID].mappedFP);
			fread(&tagID, sizeof(int), 1, (*tagLibraryMT)[threadID].mappedFP);

			int forwardContigIndex = id2Index(forwardPosition.id);
			auto itr = contigTagCounterUchar[tagID % numThread].find(std::make_pair(tagID, forwardContigIndex));
			if (itr != contigTagCounterUchar[tagID % numThread].end())
				itr->second = std::min(itr->second + 1, UCHAR_MAX - 1);
			else
				contigTagCounterUchar[tagID % numThread][std::make_pair(tagID, forwardContigIndex)] = 1;

			int reverseContigIndex = id2Index(reversePosition.id);
			itr = contigTagCounterUchar[tagID % numThread].find(std::make_pair(tagID, reverseContigIndex));
			if (itr != contigTagCounterUchar[tagID % numThread].end())
				itr->second = std::min(itr->second + 1, UCHAR_MAX - 1);
			else
				contigTagCounterUchar[tagID % numThread][std::make_pair(tagID, reverseContigIndex)] = 1;
		}
	}
    ...
    # pragma omp parallel for schedule(static, 1)
	for (long threadID = 0; threadID < numThread; ++threadID) {
		for (auto itr = contigTagCounterUchar[threadID].begin(); itr != contigTagCounterUchar[threadID].end();) {
			if (itr->second >= UCHAR_MAX - 1) {
				contigTagCounterUint[threadID][itr->first] = 0;
				itr = contigTagCounterUchar[threadID].erase(itr);
			}
			else {
				++itr;
			}
    ...
    for (long threadID = 0; threadID < numThread; ++threadID) {
		while (fread(&forwardPosition, sizeof(platanus::Position), 1, (*tagLibraryMT)[threadID].mappedFP)) {
			...
			int forwardContigIndex = id2Index(forwardPosition.id);
			auto itr = contigTagCounterUint[tagID % numThread].find(std::make_pair(tagID, forwardContigIndex));
			if (itr != contigTagCounterUint[tagID % numThread].end())
				itr->second = std::min(itr->second + 1, UINT_MAX - 1);

			int reverseContigIndex = id2Index(reversePosition.id);
			itr = contigTagCounterUint[tagID % numThread].find(std::make_pair(tagID, reverseContigIndex));
			if (itr != contigTagCounterUint[tagID % numThread].end())
				itr->second = std::min(itr->second + 1, UINT_MAX - 1);
        }
    }
...
void ScaffoldGraph::countMappedTagForEachScaffold(long numThread)
{
	...
	for (long threadID = 0; threadID < numThread; ++threadID) {
		for (auto contigItr = contigTagCounterUchar[threadID].begin(); contigItr != contigTagCounterUchar[threadID].end(); ++contigItr) {
			if (contigPositionInScaffold[contigItr->first.second].id == 0)
				continue;

			int nodeIndex = abs(contigPositionInScaffold[contigItr->first.second].id) - 1;
    ...
    # pragma omp parallel for schedule(static, 1)
	for (long threadID = 0; threadID < numThread; ++threadID) {
		for (auto itr = nodeTagCounterUchar[threadID].begin(); itr != nodeTagCounterUchar[threadID].end();) {
			if (itr->second >= UCHAR_MAX - 1) {
				nodeTagCounterUint[threadID][itr->first] = 0;
				itr = nodeTagCounterUchar[threadID].erase(itr);
			}
			else if (itr->second < this->minTagReadToLoad) {
				itr = nodeTagCounterUchar[threadID].erase(itr);
			}
			else {
				++itr;
    ...
    for (long nodeID = 0; nodeID < numNode; ++nodeID) {
        for (long edgeID1 = 0; edgeID1 < node[nodeID].numEdge - 1; ++edgeID1) {
            for (long edgeID2 = edgeID1 + 1; edgeID2 < node[nodeID].numEdge; ++edgeID2) {
                ...
                const GraphNode &node1 = (node[abs(edge1.end) - 1]);
                const GraphNode &node2 = (node[abs(edge2.end) - 1]);
//std::cout << nodeID + 1 << ", " << edge1.end << ", " << edge2.end << std::endl;
                if (!this->checkDeleteEdge(edge1, edge2, node1, node2)) continue;
...
void ScaffoldGraph::printBinSeqConstLineLength(const vector<char> &seq, std::ofstream &out)
{
	unsigned long j;
...
void ScaffoldGraph::printResultSeq(const string &outFilename)
{
    ...
    for (long i = 0; i < numNode; ++i) {
		if (resultSeq[i].seq.size() <= 0 || resultSeq[i].redundantFlag || (this->node[i].state & SC_DEL))
			continue;
...
void ScaffoldGraph::mapBubbleToResultSeq()
{
	...
	while (readNodeSeq(bubbleSeq, bubbleContigID, bubbleFP)) {
		...
		for (auto itr = bubbleOpositeContigID.begin(); itr != bubbleOpositeContigID.end(); ++itr) {
			if (*itr != 0 && contigPositionInScaffold[abs(*itr) - 1].id != 0) {
				mappedNodeID = sign(*itr) * contigPositionInScaffold[abs(*itr) - 1].id;
				mappedNodeOffset = node[abs(mappedNodeID) - 1].contig[contigPositionInScaffold[abs(*itr) - 1].offset].start;
				break;
			}
		}
		if (mappedNodeID == 0)
...
void ScaffoldGraph::updateNodeClusterID()
{
	for (auto nodeIt = node.begin(); nodeIt != node.end(); ++nodeIt) {
		...
		for (auto contigIt = nodeIt->contig.begin(); contigIt != nodeIt->contig.end(); ++contigIt) {
			if (clusterID[std::abs(contigIt->id) - 1] != 0) {
				nodeIt->clusterID = clusterID[std::abs(contigIt->id) - 1];
				break;
			}
		}
	}
