...
protected:
	...
	long minAlignmentLength;
	long numMappedLongRead;
	long totalMappedLongRead;
	std::vector<long> numLongReadUpperTail;
	std::vector<long> lengthLongReadUpperTail;
	std::vector<double> contigLongReadCoverage;
...
//		bool operator <(const MapInfoForGraph &a) const { return (position < a.position); }
    ...
    virtual void remake(const long numNewNode, const long numContigPoolSize, FILE *scaffoldFP);
    double calcNodeLongReadCoverage(const GraphNode &node);
	...
	long estimateGapSizeMedian(const std::vector<GraphLink>& links, const unsigned index, const unsigned size) const;
	...
	void calculateLongReadUncoveredCount(std::vector<std::vector<unsigned> >& unalignedCount, std::vector<std::vector<unsigned> > &coverage, const long numThread);
	...
	bool addNumLinkAndLengthFromIDPair(long leftNodeID, long rightNodeID, long &numLink, long &edgeLength);
	bool addScoreAndLengthFromIDPair(long leftNodeID, long rightNodeID, long &numLink, long &edgeLength);
	...
	void reduceAlignmentsGreedyUnique(std::vector<platanus::LongReadAlignment> &alignments, const long readLength, const long tolerenceLength);
	void detectBreakpointBasedOnCoverage(const std::vector<unsigned>& physicalCoverage, const std::vector<unsigned>& diffCoverage, const long edgeLength, const double minCoverageRate, const double maxDiffCoverageRate, const long minMedianCoverage, const long minDiffCoverage, std::vector<char>& breakpoint, const bool longRangeFlag);
	void detectBreakpointBasedOnWindowCoverage(const std::vector<unsigned>& physicalCoverage, const long windowSize, const double diffThreshold, const double SDThreshold, std::vector<char>& breakpoint);
	void maskGapSideBasedOnUncoveredCount(const std::vector<std::vector<unsigned> > &uncoveredCount, const std::vector<std::vector<unsigned> > &coverage, const long maxNumUncovered, const double maxUncoveredRate, const long minContigLength, const long numThread);
	...
	void storeBreakpointBasedOnCoverage(const std::vector<std::vector<unsigned> >& physicalCoverage, const std::vector<std::vector<unsigned> >& diffCoverage, const long edgeLength, const bool longRangeFlag, const long numThread);
	void storeBreakpointBasedOnWindowCoverage(const std::vector<std::vector<unsigned> >& physicalCoverage, const long windowSize, const long numThread);
	double calcExpectedLinkLongRead(const double coverage, const long gapSize) const;
...
public:
	...
	void joinUnambiguousNodePairIterative(const long numThread, const bool clusterFlag=false);
	...
	void divideErroneousNodeBaseLevel(const long pairLibraryBegin, const long pairLibraryEnd, const long readLength, const bool longRangeFlag, const bool storeOnlyFlag, const long numThread);
	void maskUncoveredBaseLevel(const long numThread);
	...
	void deleteInterOTUEdge(const bool unclassifiedOnlyFlag=false);
	...
	void loadLongReadLink(const long argMinAlignmentLength, const double minCoverage, const double minIdentity, const long tolerenceLength, const bool uniqueFlag, const long numThread);
	void dumpLongestNodeCoverage(std::vector<std::vector<unsigned> >& coverage, const long numOutputNode, std::string &outputFilename);
	void divideNodeBasedOnCoverage(const std::vector<std::vector<unsigned> >& physicalCoverage, const std::vector<std::vector<unsigned> >& diffCoverage, const long edgeLength, const bool longRangeFlag, const long numThread);
	void deleteThinEdgeBasedOnExpectedLongReadLink(const double threshold);
	long deleteErroneousEdgeBasedOnExpectedLongReadLink(const double threshold, const long numThread);
	void deleteErroneousEdgeBasedOnExpectedLongReadLinkIterative(const double threshold, const long numThread);
	void calculateLongReadContigCoverage(const long numThread);
