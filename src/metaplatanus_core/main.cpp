...
void Usage(void)
{
    cerr << "Usage: metaplatanus_core Command [options]" << endl
		<< "    Main commands: assemble, iterate, cluster_scaffold, cluster_fill" << endl
		<< "    Internal commands: gap_close, cov_trim, divide, merge, cluster" << endl;
}
...
{
    cerr << "MetaPlatanus version: " << platanus::ConstParam::VERSION << endl;
}
...
int main(int argc, char **argv)
{
    ...
    if (argc > 1) {
        ...
        if (strcmp(argv[1], "assemble") == 0) {
        ...
        } else if (strcmp(argv[1], "cov_trim") == 0) {
            command.reset(new ContigDivider());
            ...
            command.reset(new Cluster());
        } else if (strcmp(argv[1], "cluster_scaffold") == 0) {
            command.reset(new ClusterScaffold());
        } else if (strcmp(argv[1], "cluster_fill") == 0) {
            command.reset(new ClusterFill());
        }
