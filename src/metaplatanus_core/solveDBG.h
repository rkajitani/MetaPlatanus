...
private:
    ...
    virtual bool checkFileEnough(void)
    {
		...
		if (!(optionBool["-trim_cluster"] || optionBool["-trim_gap_side"] || optionBool["-divide_only"] || optionBool["-divide_only_long"]) && optionSingleArgs["-k"] == "" && optionSingleArgs["-g"] == "") {
...
public:
	...
	void readLongReadPAFfileAndSaveLink(const std::string PAFFilename, std::unordered_map<std::string, unsigned> &nameIndex, std::vector<SeqLib> &library, const long minAlignmentLength, const double minCoverage, const double minIdentity, const long tolerenceLength, const long numThread);
};
