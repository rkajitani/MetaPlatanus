...
, contig(), contigUnlinkSet(), allLibraryMT(NULL), longReadLibraryMT(NULL), tagLibraryMT(NULL), targetLibraryIndex(0), coverage(), seqPool(), node()
...
void ScaffoldGraph::printResultSeq(const string &outFilename)
{
    ...
    for (long i = 0; i < resultSeq.size(); ++i) {
