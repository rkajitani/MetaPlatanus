...
public:
    ...
    unsigned long long crushBubble(void);
    ...
    template <typename NEXT> void saveContig(const unsigned nextKmer, const double occurrenceRatio, const unsigned long long minOccurrence, DoubleHash<typename NEXT::keyType, unsigned short> &table);
    long saveContigSimple(FILE *contigFP) const;
	void saveJunction(FILE *contigFP) const;
    void cutBranchIterative(const unsigned long long numThread=1);
    void crushBubbleIterative(void);
    void saveEdgeKmer(Counter<KMER> &counter, const unsigned nextKmerLength) const;
    void saveAllKmer(Counter<KMER> &counter) const;
    void saveDeadEndKmer(Counter<KMER> &counter) const;
    ...
    void extractReadLinkingDeadEnds(FILE *inReadFP, FILE **outReadFP);
};
...
//////////////////////////////////////////////////////////////////////////////////////
template <typename KMER>
unsigned long long BruijnGraph<KMER>::crushBubble(void)
{
    typedef unsigned long long u64_t;
    u64_t numCrush = 0;
    ...
    for (auto it = junctionTable.begin(), end = junctionTable.end(); it != end; ++it) {
        ...
        for (unsigned char base1 = 0; base1 < 3; ++base1) {
            ...
            for (unsigned char base2 = base1 + 1; base2 < 4; ++base2) {
                if (straight[base2] == NULL || junction[base1] == NULL || junction[base1] != junction[base2]) continue;
                alignmentThreshold = static_cast<long>((std::max(straight[base1]->length, straight[base2]->length) + kmerLength - 1) * bubbleThreshold + 0.5);

                if (std::min(straight[base1]->length, straight[base2]->length) + 1 <= kmerLength) {
                    if (abs(static_cast<long>(straight[base1]->length) - straight[base2]->length) > alignmentThreshold) continue;
...
void BruijnGraph<KMER>::saveContig(const unsigned nextKmer, const double occurrenceRatio, const unsigned long long minOccurrence, DoubleHash<typename NEXT::keyType, unsigned short> &table)
{
//    unsigned long long diff = nextKmer - kmerLength;
    ...
    for (auto tableIt = leftStraightTable.begin(); tableIt != tableEnd; ++tableIt) {
                ...
                if (platanus::FlagCount(junction->out >> 4) == 1) {
                    ...
                    if (tmpStraight != NULL) {
                        ...
                        for (unsigned i = 0; i < exLength; ++i) {
                            // tmpStraight terminal is same seq in straight
                            str.set(exLength - 1- i, tmpStraight->get(kmerLength + exLength - 2 - i));
                        }
        ...
        if (writeStraight.length < nextKmer)
			continue;
        ...
        for (unsigned long long i = 0; i < writeStraight.length - nextKmer + 1; ++i) {
            ...
            unsigned short occurrence = static_cast<unsigned short>(std::max(static_cast<unsigned long long>(writeStraight.coverage * occurrenceRatio + 0.5), minOccurrence));
    ...
    for (auto tableIt = junctionTable.begin(), tableEnd = junctionTable.end(); tableIt != tableEnd; ++tableIt) {
		...
		NEXT next(nextKmer);
        binstr_t str(nextKmer);
        KMER(nextKmer).convertKeyToBinstr(str, tableIt->first);

        for (unsigned i = 0; i < nextKmer; ++i) {
            next.setForward(i, str.get(i));
            next.setReverse(nextKmer - i - 1, 0x3 ^ str.get(i));
		}
		selectedKey = std::min(next.forward,  next.reverse);
		unsigned short occurrence = static_cast<unsigned short>(std::max(static_cast<unsigned long long>(junction.coverage * occurrenceRatio + 0.5), minOccurrence));
		auto it = table.find_any(selectedKey);
		if (it->second < occurrence) {
			it->second = occurrence;
			it->first = selectedKey;
...
template <typename KMER>
long BruijnGraph<KMER>::saveContigSimple(FILE *contigFP) const
{
	long numSeq = 0;
    ...
    for (auto tableIt = leftStraightTable.begin(), tableEnd = leftStraightTable.end(); tableIt != tableEnd; ++tableIt) {
        ...
        fwrite(&(straight.coverage), sizeof(unsigned short), 1, contigFP);
...
template <typename KMER>
void BruijnGraph<KMER>::saveJunction(FILE *contigFP) const
{
    if (contigFP == NULL) {
        platanus::makeTemporaryFile();
    }
    ...
    for (auto tableIt = junctionTable.begin(), tableEnd = junctionTable.end(); tableIt != tableEnd; ++tableIt) {
		...
		unsigned short coverage = junction.coverage;
		KMER kmer(this->kmerLength);
...
//////////////////////////////////////////////////////////////////////////////////////
template <typename KMER>
void BruijnGraph<KMER>::crushBubbleIterative(void)
{
    ...
    do {
        numCrush = this->crushBubble();
        std::cerr << "NUM_CRUSH=" << numCrush << std::endl;
        concatinateNodes();
        totalCrush += numCrush;
    } while (numCrush > 0);
    std::cerr << "TOTAL_NUM_CRUSH=" << totalCrush << std::endl;
...
template <typename KMER>
void BruijnGraph<KMER>::saveAllKmer(Counter<KMER> &counter) const
{
    const unsigned long long mask = kmerLength >= 32 ? ~static_cast<unsigned long long>(0) : ~(~static_cast<unsigned long long>(0) << (2 * kmerLength));
    ...
    for (auto tableIt = leftStraightTable.begin(); tableIt != tableEnd; ++tableIt) {
        const Straight &straight = tableIt->second;
        if (straight.coverage == UINT16_MAX || straight.coverage == 0)
            continue;

		for (unsigned i = 0; i < kmerLength - 1; ++i) {
			key.setForward(kmerLength - 2 - i, straight.get(straight.length + kmerLength - 2 - i));
			key.setReverse(i + 1, 0x3 ^ straight.get(straight.length + kmerLength - 2 - i));
		}

		for (unsigned i = 0; i < straight.length; ++i) {
			key.forward <<= 2;
			key.maskForward(mask);
			key.setForward(0, straight.get(straight.length - 1 - i));
			key.reverse >>= 2;
			key.setReverse(kmerLength - 1, 0x3 ^ straight.get(straight.length - 1 - i));

			counter.setOccurrenceValue(std::min(key.forward, key.reverse), straight.coverage);
		}
    }

    for (auto tableIt = junctionTable.begin(), tableEnd = junctionTable.end(); tableIt != tableEnd; ++tableIt) {
        const Junction &junction = tableIt->second;
        if (junction.coverage == UINT16_MAX || junction.coverage == 0) {
            continue;
        }

		KMER kmer(kmerLength);
		kmer.forward = tableIt->first;
        binstr_t seq(kmerLength);
        KMER(kmerLength).convertKeyToBinstr(seq, tableIt->first);

		for (unsigned i = 0; i < kmerLength; ++i) {
			key.setForward(i, seq.get(i));
			key.setReverse(i, 0x3 ^ seq.get(kmerLength - 1 - i));
		}

		counter.setOccurrenceValue(std::min(key.forward, key.reverse), junction.coverage);
    }
}


template <typename KMER>
void BruijnGraph<KMER>::saveDeadEndKmer(Counter<KMER> &counter) const
{
    const unsigned long long mask = kmerLength >= 32 ? ~static_cast<unsigned long long>(0) : ~(~static_cast<unsigned long long>(0) << (2 * kmerLength));
    ...
    for (auto tableIt = leftStraightTable.begin(); tableIt != tableEnd; ++tableIt) {
		...
		for (unsigned i = 0; i < kmerLength; ++i) {
			leftKey.setForward(kmerLength - 1 - i, straight.get(kmerLength - 1 - i));
			leftKey.setReverse(i, 0x3 ^ straight.get(kmerLength - 1 - i));
			rightKey.setForward(kmerLength - 1 - i, straight.get(straight.length + kmerLength - 2 - i));
			rightKey.setReverse(i, 0x3 ^ straight.get(straight.length + kmerLength - 2 - i));
		}

        if (platanus::FlagCount(straight.out >> 4) == 1)
			counter.setOccurrenceValue(std::min(leftKey.forward, leftKey.reverse), straight.coverage);

		if (platanus::FlagCount(straight.out & 0xf) == 1)
			counter.setOccurrenceValue(std::min(rightKey.forward, rightKey.reverse), straight.coverage);
    }
}
...
template <typename KMER>
void BruijnGraph<KMER>::extractReadLinkingDeadEnds(FILE *inReadFP, FILE **outReadFP)
{
    const unsigned long long mask = kmerLength >= 32 ? ~static_cast<unsigned long long>(0) : ~(~static_cast<unsigned long long>(0) << (2 * kmerLength));
    platanus::SEQ seq;
    KMER kmer(this->kmerLength);
    *outReadFP = platanus::makeTemporaryFile();
    rewind(inReadFP);
    
    while (seq.readTemporaryFile(inReadFP)) {
        if (seq.length < this->kmerLength)
			continue;

        platanus::SEQ tmpSeq = seq;
        ...
        const Straight *straight1 = NULL;
        const Straight *straight2 = NULL;

		bool linkFlag = false;
		long start = tmpSeq.length;
		long end = 0;

        for (unsigned long long i = 0; i < this->kmerLength - 1; ++i) {
            kmer.setForward(this->kmerLength - i - 2, tmpSeq.base[i]);
            kmer.setReverse(i + 1, 0x3 ^ tmpSeq.base[i]);
        }
        for (long i = 0; i < tmpSeq.length - this->kmerLength + 1; ++i) {
            kmer.forward <<= 2;
            kmer.maskForward(mask);
            kmer.setForward(0, tmpSeq.base[i + this->kmerLength - 1]);
            kmer.reverse >>= 2;
            kmer.setReverse(this->kmerLength - 1, 0x3 ^ tmpSeq.base[i + this->kmerLength - 1]);
            ...
            if ((straight2 = findStraightHead(kmer.forward)) != NULL || (straight2 = findStraightHead(kmer.reverse)) != NULL) {
                if (straight2->out >> 4)
					continue;

                if (straight1 != NULL && straight1 != straight2) {
					linkFlag = true;
					start = std::min(i, start);
					end = std::max(i + (unsigned)kmerLength, end);
                }
				else {
                    straight1 = straight2;
					start = std::min(i, start);
			...
			else if ((straight2 = findStraightTail(kmer.forward)) != NULL || (straight2 = findStraightTail(kmer.reverse)) != NULL) {
                if (straight2->out & 0xf)
					continue;
		...
		if (linkFlag) {
			tmpSeq.length = end - start;
			tmpSeq.base.resize(tmpSeq.length);
			for (unsigned j = start; j < end; ++j)
				tmpSeq.base[j - start] = seq.base[j];

			tmpSeq.positionUnknown.clear();
			tmpSeq.numUnknown = 0;
			for (unsigned j = 0; j < seq.numUnknown; ++j) {
				if (seq.positionUnknown[j] >= start && seq.positionUnknown[j] < end) {
					tmpSeq.positionUnknown.push_back(seq.positionUnknown[j] - start);
					++(tmpSeq.numUnknown);
				}
			}

			tmpSeq.writeTemporaryFile(*outReadFP);
		}
    }
}
