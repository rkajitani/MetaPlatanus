...
const unsigned PairedDBG::MIN_LENGTH_FOR_MGC = 10000;
...
void PairedDBG::storeGraphLinkFromOverlap(vector<GraphLinkWithFlag> &graphLinkPool)
{
    ...
    for (unsigned d = 0; d < TABLE_DIVID; ++d) {
		...
		for (; overlapIterator != overlapEnd; ++overlapIterator) {
            ...
            if (contigPositionInScaffold[j].id == 0)
				continue;
...
//    cerr << "linking scaffolds (MIN_LINK = " << minLink << ")" << endl;
	...
	if (graphLinkPool[0].overlapFlag) {
		...
		indexes.back().gap = graphLinkPool[0].gap;
	}
...
//		graphLink.gap = estimateGapSizeAverage(targetLinkPool, 0, numLink);
		graphLink.gap = estimateGapSizeMedian(targetLinkPool, 0, numLink);
	}
...
long PairedDBG::estimateGapSizeMedian(const std::vector<GraphLink>& links, const unsigned index, const unsigned size) const
{
	if (size == 1)
		return links[index].gap;

	std::vector<long> buf(size);

	for (long i = 0; i < size; ++i)
		buf[i] = links[index + i].gap;

	std::partial_sort(buf.begin(), buf.begin() + size / 2 + 1, buf.end());

	if (size % 2 == 1)
		return buf[size / 2];
	else
		return (buf[size / 2 - 1] + buf[size / 2]) / 2 + 0.5;
}
...
void PairedDBG::resetGraph()
{
	...
	for (unsigned long i = 0; i < this->contigState.size(); ++i) {
		contigState[i] = 0;
		if (!(oppositeBubbleContigID.empty()))
...
//				continue;
			...
			std::array<long, 2> edgeLengthForHaplotype;
			std::array<long, 2> numEdgeForHaplotype;
			std::array<long, 2> centerLength;
			sumLinkForHaplotype.fill(0);
			edgeLengthForHaplotype.fill(0);
			numEdgeForHaplotype.fill(0);
			centerLength.fill(this->node[centerNodeIndex].length - 2*(this->contigMaxK - 1));
			...
			if (resolutionMode == SCORE) {
				for (long leftIndex = 0; leftIndex < 2; ++leftIndex) {
					for (long rightIndex = 0; rightIndex < 2; ++rightIndex) {
						const int h = (leftIndex == rightIndex);
						if (addScoreAndLengthFromIDPair(externalNodeID[0][leftIndex], externalNodeID[1][rightIndex], sumLinkForHaplotype[h], edgeLengthForHaplotype[h]))
							++numEdgeForHaplotype[h];
					}
				}
			}
			else {
				for (long leftIndex = 0; leftIndex < 2; ++leftIndex) {
					for (long rightIndex = 0; rightIndex < 2; ++rightIndex) {
						const int h = (leftIndex == rightIndex);
						if (addNumLinkAndLengthFromIDPair(externalNodeID[0][leftIndex], externalNodeID[1][rightIndex], sumLinkForHaplotype[h], edgeLengthForHaplotype[h]))
							++numEdgeForHaplotype[h];
					}
				}
			}
			...
			char crossFlag;
			if (linkRateThreshold * sumLinkForHaplotype[0] >= sumLinkForHaplotype[1] && numEdgeForHaplotype[0] > 0)
				crossFlag = 1;
			else if (linkRateThreshold * sumLinkForHaplotype[1] >= sumLinkForHaplotype[0] && numEdgeForHaplotype[1] > 0)
				crossFlag = 0;
			else
				continue;

			if ((resolutionMode == LINK || resolutionMode == SCORE) && (std::abs(centerLength[!crossFlag] - edgeLengthForHaplotype[!crossFlag] / numEdgeForHaplotype[!crossFlag]) > this->tolerence))
				continue;
...
//				continue;
			...
			edgeLengthForHaplotype.fill(0);
			numEdgeForHaplotype.fill(0);
			centerLength.fill(this->node[centerNodeIndex].length);
			if (resolutionMode == TAG) {
				if (node[centerNodeIndex].length > maxFragmentLengthOfTag)
					continue;
			...
			if (resolutionMode == SCORE) {
				for (long leftIndex = 0; leftIndex < 2; ++leftIndex) {
					for (long rightIndex = 0; rightIndex < 2; ++rightIndex) {
						const int h = (leftIndex == rightIndex);
						if (addScoreAndLengthFromIDPair(externalNodeID[0][leftIndex].id, externalNodeID[1][rightIndex].id, sumLinkForHaplotype[h], edgeLengthForHaplotype[h])) {
							++numEdgeForHaplotype[h];
							centerLength[h] += externalNodeID[0][leftIndex].gap + externalNodeID[1][rightIndex].gap;
						}
					}
				}
			}
				...
				for (long leftIndex = 0; leftIndex < 2; ++leftIndex) {
					for (long rightIndex = 0; rightIndex < 2; ++rightIndex) {
						const int h = (leftIndex == rightIndex);
						if (addNumLinkAndLengthFromIDPair(externalNodeID[0][leftIndex].id, externalNodeID[1][rightIndex].id, sumLinkForHaplotype[h], edgeLengthForHaplotype[h])) {
							++numEdgeForHaplotype[h];
							centerLength[h] += externalNodeID[0][leftIndex].gap + externalNodeID[1][rightIndex].gap;
						}
					}
			...
			if ((resolutionMode == LINK || resolutionMode == TAG) &&
				((std::max(sumLinkForHaplotype[0], sumLinkForHaplotype[1]) < this->minLink) ||
				(sumLinkForHaplotype[0] > 0 && std::abs(centerLength[0] - edgeLengthForHaplotype[0] / sumLinkForHaplotype[0]) > this->tolerence) ||
				(sumLinkForHaplotype[1] > 0 && std::abs(centerLength[1] - edgeLengthForHaplotype[1] / sumLinkForHaplotype[1]) > this->tolerence))) {
					continue;
			}
...
double PairedDBG::calcNodeLongReadCoverage(const GraphNode &node)
{
    unsigned long long sum = 0;
    unsigned long long num = 0;
    for (long i = 0; i < node.numContig; ++i) {
		...
		if (contigPositionInScaffold[contigIndex].id != 0) {
			num += contig[contigIndex].length;
			sum += contigLongReadCoverage[contigIndex] * contig[contigIndex].length;
		}
    }
...
{
	resultSeq.clear();
	nodeBreakpointPosition.resize(contig.size());

	bool clusterFlag = false;
    for (long contigIndex = 0; contigIndex < contig.size(); ++contigIndex) {
		if (contigIndex < node.size() && node[contigIndex].clusterID != 0) {
			clusterFlag = true;
			break;
		}
	}
    ...
    for (long contigIndex = 0; contigIndex < contig.size(); ++contigIndex) {
		...
		long clusterID = 0;
		if (clusterFlag)
			clusterID = node[contigIndex].clusterID;
...
void PairedDBG::joinUnambiguousNodePairIterative(const long numThread, const bool clusterFlag)
{
	...
	for (unsigned long nodeIndex = 0; nodeIndex < this->node.size(); ++nodeIndex) {
		...
		if (contigRef.size() > 1) {
			...
			if (contigRef[contigRef.size() - 1].start - contigRef[contigRef.size() - 2].end > contigRef[contigRef.size() - 1].end - contigRef[contigRef.size() - 1].start)
				breakpointFlag[contigRef.size() - 1] = 1;
		}
...
void PairedDBG::makeScaffold(void)
{
    ...
    for (long i = 0; i < numNode; ++i) {
        ...
        while (candidate.size() > 0) {
            ...
            long closest = abs(candidate[0].start);
...
void PairedDBG::remake(const long numNewNode, const long newContigPoolSize, FILE *scaffoldFP)
{
    this->destroyGraph();
    numNode = numNewNode;
    node.resize(numNode);
    ...
    for (long i = 0; i < numNode; ++i) {
		...
		node[i].coverage = calcNodeCoverage(node[i]);
		if (contigLongReadCoverage.empty())
			node[i].longReadCoverage = 0.0;
		else
			node[i].longReadCoverage = calcNodeLongReadCoverage(node[i]);
    }
...
bool PairedDBG::addNumLinkAndLengthFromIDPair(long leftNodeID, long rightNodeID, long &numLink, long &edgeLength)
{
	GraphNode &leftNode = this->node[abs(leftNodeID) - 1];
	char directionFromLeft = sign(leftNodeID);
	char rightStrand = directionFromLeft * sign(rightNodeID);
	...
	for (long i = 0; i < leftNode.numEdge; ++i) {
		if (abs(leftNode.edge[i].end) == abs(rightNodeID) &&
			...
			numLink += leftNode.edge[i].numLink;
			edgeLength += leftNode.edge[i].length;
			return true;
		}
	}
...
bool PairedDBG::addScoreAndLengthFromIDPair(long leftNodeID, long rightNodeID, long &score, long &edgeLength)
{
	GraphNode &leftNode = this->node[abs(leftNodeID) - 1];
	char directionFromLeft = sign(leftNodeID);
	char rightStrand = directionFromLeft * sign(rightNodeID);
...
void PairedDBG::setClusterIDfromSeqName(const platanus::Contig &contig)
{
	...
	for (long i = 0; i < this->numContig; ++i) {
		...
		if (pos != std::string::npos && pos + CLUSTER_PREFIX.size() < contig.name[i].size())
...
void PairedDBG::deleteInterOTUEdge(const bool unclassifiedOnlyFlag)
{
	Mgc mgc(4);
    vector<long> ids;
    ...
    for (long nodeID = 0 ; nodeID < numNode; ++nodeID) {
        ...
        for (long edgeID = 0; edgeID < node1.numEdge; ++edgeID) {
			...
			if (unclassifiedOnlyFlag && node1.clusterID * node2.clusterID != 0)
				continue;
...
void PairedDBG::loadLongReadLink(const long argMinAlignmentLength, const double minCoverage, const double minIdentity, const long tolerenceLength, const bool uniqueFlag, const long numThread)
{
	this->minAlignmentLength = argMinAlignmentLength;
    omp_set_num_threads(numThread);
	...
	for (unsigned threadIndex = 0; threadIndex < numThread; ++threadIndex) {
		...
		while (fread(&readLength, sizeof(long), 1, (*longReadLibraryMT)[threadIndex].alignmentFP)) {
			...
			if (uniqueFlag)
				reduceAlignmentsGreedyUnique(alignmentBuffer, readLength, tolerenceLength);
			else
			...
			for (unsigned j = 0; j < alignmentBuffer.size(); ++j) {
				...
				if ((double)alignmentBuffer[j].match / alignmentLength >= minIdentity &&
					(alignmentLength >= this->minAlignmentLength ||
...
void PairedDBG::reduceAlignmentsGreedyUnique(vector<platanus::LongReadAlignment> &alignments, const long readLength, const long tolerenceLength)
{
	std::sort(alignments.begin(), alignments.end(), platanus::LongReadAlignment::LongReadAlignmentGreater()); 
	
	vector<long> deleteIndex; 
	unsigned long numRetained = alignments.size();
	for (unsigned long i = 1; i < alignments.size(); ++i) {
		for (unsigned long j = 0; j < i; ++j) {
			if (std::min(alignments[i].readEnd - alignments[j].readStart, alignments[j].readEnd - alignments[i].readStart) > tolerenceLength) {
				deleteIndex.push_back(i);
				if (2 * alignments[i].score > alignments[j].score)
					deleteIndex.push_back(j);
			}
		}
	}

	for (auto itr = deleteIndex.begin(); itr != deleteIndex.end(); ++itr) {
		if (alignments[*itr].score != INT_MIN) {
			alignments[*itr].score = INT_MIN;
			--numRetained;
		}
	}
...
void PairedDBG::divideErroneousNodeBaseLevel(const long pairLibraryBegin, const long pairLibraryEnd, const long readLength, const bool longRangeFlag, const bool storeOnlyFlag, const long numThread)
{
    const long WINDOW_SIZE = 100000;
	...
	if (longReadLibraryMT != NULL) {
		this->setTolerence((*longReadLibraryMT)[0].getAverageInsSize());
		const long edgeLength = this->contigMaxK;

		vector<vector<unsigned> > physicalCoverage(numNode);
		vector<vector<unsigned> > diffCoverage(numNode);
		for (unsigned i = 0; i < numNode; ++i) {
			physicalCoverage[i].assign(node[i].length, 0);
			diffCoverage[i].assign(node[i].length, 0);
		}
...
*/
		...
		if (storeOnlyFlag) {
			this->storeBreakpointBasedOnCoverage(physicalCoverage, diffCoverage, edgeLength, longRangeFlag, numThread);
			if (longRangeFlag)
				this->storeBreakpointBasedOnWindowCoverage(physicalCoverage, WINDOW_SIZE, numThread);
		}
		else
			this->divideNodeBasedOnCoverage(physicalCoverage, diffCoverage, edgeLength, longRangeFlag, numThread);
	}
...
/*
	if (allLibraryMT != NULL) {
		for (unsigned libraryID = pairLibraryBegin; libraryID < pairLibraryEnd; ++libraryID) {
			this->setTargetLibraryIndex(libraryID);
			this->setTolerence(minTolerenceFactor * (*allLibraryMT)[libraryID][0].getSDInsSize());
			const long edgeLength = (*allLibraryMT)[libraryID][0].getAverageInsSize();
...
//				this->calculateDiffCoverage(diffCoverage, (*allLibraryMT)[libraryID][0].getAverageInsSize(), (*allLibraryMT)[libraryID][0].getAverageInsSize(), numThread);
				this->divideNodeBasedOnCoverage(physicalCoverage, diffCoverage, edgeLength, numThread);
			}
		}
...
void PairedDBG::maskUncoveredBaseLevel(const long numThread)
{
	const long MAX_NUM_UNCOVERED = 2;
	const double MAX_UNCOVERED_RATE = 0.5;

	cerr << "masking uncovered regions based on base-level coverages ..." << endl;
	...
	if (longReadLibraryMT != NULL) {
		...
		vector<vector<unsigned> > coverage(numNode);
		vector<vector<unsigned> > uncoveredCount(numNode);
		for (unsigned i = 0; i < numNode; ++i) {
			coverage[i].assign(node[i].length, 0);
			uncoveredCount[i].assign(node[i].length, 0);
		}

		this->calculateLongReadUncoveredCount(uncoveredCount, coverage, numThread);
		this->maskGapSideBasedOnUncoveredCount(uncoveredCount, coverage, MAX_NUM_UNCOVERED, MAX_UNCOVERED_RATE, this->contigMaxK, numThread);
	}
}
    ...
    for (long threadID = 0; threadID < numThread; ++threadID) {
		...
		while (fread(&numPairLink, sizeof(unsigned), 1, (*allLibraryMT)[targetLibraryIndex][threadID].mappedFP)) {
			...
			for (unsigned pairIndex = 0; pairIndex < numPairLink; ++pairIndex) {
				...
				long lockIndex = nodeIndex % NUM_OMP_LOCK;
				omp_set_lock(&lock[lockIndex]);
				for (long i = range.first; i <= range.second; ++i) {
					++(physicalCoverage[nodeIndex][i]);
				}
				omp_unset_lock(&lock[lockIndex]);
...
void PairedDBG::calculateLongReadPhysicalCoverage(vector<vector<unsigned> >& physicalCoverage, const long numThread)
{
    ...
    for (long threadID = 0; threadID < numThread; ++threadID) {
		...
		while (fread(&numResult, sizeof(unsigned), 1, (*longReadLibraryMT)[threadID].mappedReadFP)) {
			...
			std::unordered_map<int, vector<std::pair<int, int> > > index2Range;
			...
			for (unsigned long i = 0; i < numResult; ++i) {
				...
				if (mapItr == index2Range.end())
					index2Range.emplace(nodeIndex, vector<std::pair<int, int> >(1, std::make_pair(targetStart, targetEnd)));
				else
					mapItr->second.emplace_back(targetStart, targetEnd);
			}

			for (auto mapItr = index2Range.begin(); mapItr != index2Range.end(); ++mapItr) {
				std::stable_sort(mapItr->second.begin(), mapItr->second.end(), platanus::PairFirstLess());

				auto vecItr = mapItr->second.begin();
				long currentStart = vecItr->first;
				long currentEnd = vecItr->second;
				long maxStart = currentStart;
				long maxEnd = currentEnd;
				long preEnd = currentEnd;

				++vecItr;
				for (; vecItr != mapItr->second.end(); ++vecItr) {
					long localStart = vecItr->first;
					long localEnd = vecItr->second;
					if (localStart - preEnd + 1 > readLength) {
						...
						currentStart = std::min(currentStart, localStart);
						currentEnd = std::max(currentEnd, localEnd);
					}
				...
				long lockIndex = mapItr->first % NUM_OMP_LOCK;
				omp_set_lock(&lock[lockIndex]);
...
void PairedDBG::calculateLongReadUncoveredCount(vector<vector<unsigned> > &uncoveredCount, vector<vector<unsigned> > &coverage, const long numThread)
{
	const long NUM_OMP_LOCK = 65536;
			...
			for (auto mapItr = index2Range.begin(); mapItr != index2Range.end(); ++mapItr) {
				...
				for (; vecItr != mapItr->second.end(); ++vecItr) {
					...
					if (localStart - preEnd + 1 > readLength) {
						...
						currentStart = localStart;
						currentEnd = localEnd;
						currentStartIndex = vecItr - mapItr->second.begin();
						currentEndIndex = currentStartIndex;
					}
					else {
						currentStart = std::min(currentStart, localStart);
						currentEnd = std::max(currentEnd, localEnd);
						currentEndIndex = vecItr - mapItr->second.begin();
					}
				...
				long lockIndex = mapItr->first % NUM_OMP_LOCK;
				omp_set_lock(&lock[lockIndex]);
				for (long i = maxStartIndex; i < maxEndIndex; ++i) {
					for (long j = mapItr->second[i].first; j < mapItr->second[i].second; ++j)
						++(coverage[mapItr->first][j]);
					for (long j = mapItr->second[i].second; j < mapItr->second[i + 1].first; ++j)
						++(uncoveredCount[mapItr->first][j]);
				}
				for (long j = mapItr->second[maxEndIndex].first; j < mapItr->second[maxEndIndex].second; ++j)
					++(coverage[mapItr->first][j]);
				omp_unset_lock(&lock[lockIndex]);
			}
        }
...
void PairedDBG::calculateLongReadContigCoverage(const long numThread)
{
	const long NUM_OMP_LOCK = 65536;
	...
	long numMappedRead = 0;
	long totalMappedRead = 0;
	vector<vector<long> > histoThread(numThread);
	this->contigLongReadCoverage.assign(this->contig.size(), 0.0);
	vector<long> numMappedReadContig(this->contig.size(), 0);

	# pragma omp parallel for schedule(static, 1) reduction(+: numMappedRead, totalMappedRead)
    for (long threadID = 0; threadID < numThread; ++threadID) {
		unsigned numResult;
		int targetStart;
		int targetEnd;
		...
		while (fread(&numResult, sizeof(unsigned), 1, (*longReadLibraryMT)[threadID].mappedReadFP)) {
			...
			for (unsigned long i = 0; i < numResult; ++i) {
				...
				long contigIndex = id2Index(position.id);
				long lockIndex = contigIndex % NUM_OMP_LOCK;

				omp_set_lock(&lock[lockIndex]);
				this->contigLongReadCoverage[contigIndex] += abs(targetEnd - targetStart);
				++numMappedReadContig[contigIndex];
				omp_unset_lock(&lock[lockIndex]);
			}

			++numMappedRead;
			totalMappedRead += readLength;
			if (histoThread[threadID].size() <= readLength)
				histoThread[threadID].resize(readLength + 1, 0);
			++histoThread[threadID][readLength];
		}
	...
	for (long i = 0; i < this->contig.size(); ++i) {
		if (numMappedReadContig[i] > 0)
			this->contigLongReadCoverage[i] = (double)(this->contigLongReadCoverage[i]) / this->contig[i].length + 0.5;
	}

	vector<long> lengthHisto; 

    for (long threadID = 0; threadID < numThread; ++threadID) {
		if (lengthHisto.size() < histoThread[threadID].size())
			lengthHisto.resize(histoThread[threadID].size());
	}
    for (long threadID = 0; threadID < numThread; ++threadID) {
		for (long i = 0; i <  histoThread[threadID].size(); ++i)
			lengthHisto[i] += histoThread[threadID][i];
	}
	this->numLongReadUpperTail.assign(lengthHisto.size(), 0);
	this->lengthLongReadUpperTail.assign(lengthHisto.size(), 0);
	long cumNum = 0;
	long cumLength = 0;
	for (long i = lengthHisto.size() - 1; i >= 0; --i) {
		cumNum += lengthHisto[i];
		cumLength += i * lengthHisto[i];
		this->numLongReadUpperTail[i] = cumNum;
		this->lengthLongReadUpperTail[i] = cumLength;
	}

	if (numMappedLongRead > 0)
		(*longReadLibraryMT)[0].setAverageInsSize((double)(totalMappedRead) / numMappedRead + 0.5);

	this->numMappedLongRead = numMappedRead;
	this->totalMappedLongRead = totalMappedRead;
}
		...
		while (fread(&numPairLink, sizeof(unsigned), 1, mappedFP)) {
			...
			for (unsigned pairIndex = 0; pairIndex < numPairLink; ++pairIndex) {
				...
				if (node[reverseNodeIndex].length >= lengthThreshold && forwardRedundancyCheckSet.find(forwardNodeIndex) == forwardRedundancyCheckSet.end()) {
					...
					long lockIndex = forwardNodeIndex % NUM_OMP_LOCK;
					omp_set_lock(&lock[lockIndex]);
					for (long i = range.first; i <= range.second; ++i) {
				...
				if (node[forwardNodeIndex].length >= lengthThreshold && reverseRedundancyCheckSet.find(reverseNodeIndex) == reverseRedundancyCheckSet.end()) {
					...
					long lockIndex = reverseNodeIndex % NUM_OMP_LOCK;
					omp_set_lock(&lock[lockIndex]);
					for (long i = range.first; i <= range.second; ++i) {
...
void PairedDBG::detectBreakpointBasedOnCoverage(const vector<unsigned>& physicalCoverage, const vector<unsigned>& diffCoverage, const long edgeLength, const double minCoverageRate, const double maxDiffCoverageRate, const long minMedianCoverage, const long minDiffCoverage, vector<char>& breakpoint, const bool longRangeFlag)
{
...
void PairedDBG::detectBreakpointBasedOnWindowCoverage(const vector<unsigned>& physicalCoverage, const long windowSize, const double diffThreshold, const double SDThreshold, vector<char>& breakpoint)
{
	breakpoint.assign(physicalCoverage.size(), 0);

	if (breakpoint.size() <= 2*windowSize)
		return;

	std::vector<double> windowMean(physicalCoverage.size() - windowSize + 1, 0.0);
	std::vector<double> windowSD(physicalCoverage.size() - windowSize + 1, 0.0);

	for (long i = 0; i < windowSize; ++i) {
		windowMean[0] += physicalCoverage[i];
		windowSD[0] += std::pow(physicalCoverage[i], 2.0);
	}

	for (long i = 1; i < windowMean.size(); ++i) {
		windowMean[i] = windowMean[i - 1] - physicalCoverage[i - 1] + physicalCoverage[i + windowSize - 1];
		windowSD[i] = windowSD[i - 1] - std::pow(physicalCoverage[i - 1], 2.0) + std::pow(physicalCoverage[i + windowSize - 1], 2.0);
	}
	for (long i = 0; i < windowMean.size(); ++i) {
		windowMean[i] /= (double)windowSize;
		windowSD[i] = std::sqrt(windowSD[i] / windowSize - std::pow(windowMean[i], 2.0));
	}

	for (long i = 0; i < physicalCoverage.size() - 2*windowSize + 1; ++i) {
		if (!(platanus::withinRatioThreshold(windowMean[i], windowMean[i + windowSize], diffThreshold)) &&
			std::abs(windowMean[i] - windowMean[i + windowSize]) > SDThreshold * std::max(windowSD[i], windowSD[i + windowSize])) {
...
void PairedDBG::maskGapSideBasedOnUncoveredCount(const vector<vector<unsigned> > &uncoveredCount, const vector<vector<unsigned> > &coverage, const long maxNumUncovered, const double maxUncoveredRate, const long minContigLength, const long numThread) 
{
    omp_set_num_threads(numThread);

	# pragma omp parallel for schedule(dynamic)
    for (long i = 0; i < contig.size(); ++i) {
		if (contig[i].base.size() != node[i].length)
			continue;

		string newSeq = contig[i].base;
		long j = 0;
		while (j < contig[i].base.size()) {
			long gapStart = 0;
			long gapEnd = newSeq.size();

			if (contig[i].base[j] == 4) {
				gapStart = j;
				for (++j; j < contig[i].base.size(); ++j) {
					if (contig[i].base[j] != 4)
						break;
				}
				gapEnd = j;
			...
			for (long k = gapStart - 1; k >= 0; --k) {
				if (uncoveredCount[i][k] > maxNumUncovered && uncoveredCount[i][k] > maxUncoveredRate * coverage[i][k])
					newSeq[k] = 4;
				else
					break;
			}
			for (long k = gapEnd; k < contig[i].base.size(); ++k) {
				if (uncoveredCount[i][k] > maxNumUncovered && uncoveredCount[i][k] > maxUncoveredRate * coverage[i][k])
					newSeq[k] = 4;
				else
					break;
		...
		while (j < contig[i].base.size()) {
			long contigStart = 0;
			long contigEnd = newSeq.size();

			if (newSeq[j] != 4) {
				contigStart = j;
				for (++j; j < newSeq.size(); ++j) {
					if (newSeq[j] == 4)
						break;
				}
				contigEnd = j;
				if (contigEnd - contigStart < minContigLength) {
					for (long k = contigStart; k < contigEnd; ++k)
						newSeq[k] = 4;
				}
			}
			else {
...
void PairedDBG::divideNodeBasedOnCoverage(const vector<vector<unsigned> >& physicalCoverage, const vector<vector<unsigned> >& diffCoverage, const long edgeLength, const bool longRangeFlag, const long numThread)
{
	const double MIN_COVERAGE_RATE = 0.25;
	const double MAX_DIFF_COVERAGE_RATE = 1.0;
	const unsigned MIN_MEDIAN_COVERAGE = longRangeFlag ? 32 : 8;
	const unsigned MIN_DIFF_COVERAGE = 3;
	const long MIN_OVERLAP_TO_JOIN = 32;
	...
	for (unsigned threadIndex = 0; threadIndex < numThread; ++threadIndex) {
		...
		for (long i = threadIndex; i < numNode; i += numThread) {
			...
			else if (physicalCoverage[i].size() <= 2*edgeLength) {
				fwrite(&(node[i].numContig), sizeof(long), 1, scaffoldFP[threadIndex]);
				for (auto itr = node[i].contig.begin(); itr != node[i].contig.end(); ++itr)
			...
			detectBreakpointBasedOnCoverage(physicalCoverage[i], diffCoverage[i], edgeLength, MIN_COVERAGE_RATE, MAX_DIFF_COVERAGE_RATE, MIN_MEDIAN_COVERAGE, MIN_DIFF_COVERAGE, baseBreakpoint, longRangeFlag);

			bool isDivided = detectContigBoundaryBreakpoints(edgeLength, node[i], baseBreakpoint, contigBreakpoint);
...
void PairedDBG::storeBreakpointBasedOnCoverage(const vector<vector<unsigned> >& physicalCoverage, const vector<vector<unsigned> >& diffCoverage, const long edgeLength, const bool longRangeFlag, const long numThread)
{
	const double MIN_COVERAGE_RATE = 0.25;
	const double MAX_DIFF_COVERAGE_RATE = 1.0;
	const unsigned MIN_MEDIAN_COVERAGE = longRangeFlag ? 32 : 8;
	...
	for (unsigned threadIndex = 0; threadIndex < numThread; ++threadIndex) {
		vector<char> baseBreakpoint;
		for (long i = threadIndex; i < numNode; i += numThread) {
			if (node[i].numContig != 1 || physicalCoverage[i].size() <= 2*edgeLength)
				continue;

			detectBreakpointBasedOnCoverage(physicalCoverage[i], diffCoverage[i], edgeLength, MIN_COVERAGE_RATE, MAX_DIFF_COVERAGE_RATE, MIN_MEDIAN_COVERAGE, MIN_DIFF_COVERAGE, baseBreakpoint, longRangeFlag);

			bool divideFlag = false;
			for (long j = edgeLength; j < baseBreakpoint.size() - edgeLength; ++j) {
...
void PairedDBG::storeBreakpointBasedOnWindowCoverage(const vector<vector<unsigned> >& physicalCoverage, const long windowSize, const long numThread)
{
	const double DIFF_THRESHOLD = 2.0;
	...
	for (unsigned threadIndex = 0; threadIndex < numThread; ++threadIndex) {
		vector<char> baseBreakpoint;
		for (long i = threadIndex; i < numNode; i += numThread) {
			if (node[i].numContig != 1 || physicalCoverage[i].size() <= 2*windowSize)
				continue;

			detectBreakpointBasedOnWindowCoverage(physicalCoverage[i], windowSize, DIFF_THRESHOLD, SD_THRESHOLD, baseBreakpoint);

			bool divideFlag = false;
			for (long j = windowSize; j < baseBreakpoint.size() - windowSize; ++j) {
				if (baseBreakpoint[j] == 1) {
					nodeBreakpointPosition[i].push_back(j);
					divideFlag = true;
				}
...
double PairedDBG::calcExpectedLinkLongRead(const double coverage, const long gapSize) const
{
	long minLengthToLink = std::max(gapSize, 0L) + 2 * this->minAlignmentLength;

	if (minLengthToLink >= this->numLongReadUpperTail.size())
		return 0.0;

	double totalEffectiveRegion = this->lengthLongReadUpperTail[minLengthToLink] -  minLengthToLink * this->numLongReadUpperTail[minLengthToLink];
	return coverage * (totalEffectiveRegion / this->totalMappedLongRead);
}


void PairedDBG::deleteThinEdgeBasedOnExpectedLongReadLink(const double threshold)
{
    vector<long> ids;
    unsigned long long numDelete = 0;

    std::cerr << "removing thin edges based on expected numbers of links..." << std::endl;

    for (long nodeID = 0 ; nodeID < numNode; ++nodeID) {
        GraphNode &node1 = this->node[nodeID];
        for (long edgeID = 0; edgeID < node1.numEdge; ++edgeID) {
			GraphNode &node2 = this->node[id2Index(node1.edge[edgeID].end)];
			double expectedLink = calcExpectedLinkLongRead(std::min(node1.longReadCoverage, node2.longReadCoverage), node1.edge[edgeID].length);

            if (expectedLink != 0.0 && (double)(node1.edge[edgeID].numLink) / expectedLink < threshold) {
				ids.push_back(nodeID + 1);
				ids.push_back(node1.edge[edgeID].end);
                ++numDelete;
            }
...
long PairedDBG::deleteErroneousEdgeBasedOnExpectedLongReadLink(const double threshold, const long numThread)
{
    vector<long> ids;
    long numDelete = 0;
    ...
    for (long nodeID = 0; nodeID < numNode; ++nodeID) {
        for (long edgeID1 = 0; edgeID1 < node[nodeID].numEdge - 1; ++edgeID1) {
            for (long edgeID2 = edgeID1 + 1; edgeID2 < node[nodeID].numEdge; ++edgeID2) {
				...
				double expectedLink1 = calcExpectedLinkLongRead(node[nodeID].longReadCoverage, edge1.length);
				double expectedLink2 = calcExpectedLinkLongRead(node[nodeID].longReadCoverage, edge2.length);

                double normLink1 = (double)(edge1.numLink) / std::max(1.0, expectedLink1);
                double normLink2 = (double)(edge2.numLink) / std::max(1.0, expectedLink2);

                if (edge1.numLink < edge2.numLink && normLink1 <= threshold * normLink2) {
                    #pragma omp critical (delete_edge)
                    {
                        ids.push_back(nodeID + 1);
                        ids.push_back(edge1.end);
                    ...
                    ++numDelete;
                } else if (edge2.numLink < edge1.numLink && normLink2 <= threshold * normLink1) {
                    #pragma omp critical (delete_edge)
                    {
                        ids.push_back(nodeID + 1);
                        ids.push_back(edge2.end);
...
void PairedDBG::deleteErroneousEdgeBasedOnExpectedLongReadLinkIterative(const double threshold, const long numThread)
{
    cerr << "removing erroneous edges..." << endl << endl;;
    long totalDelete = 0;
    long numDelete;
    do {
        numDelete = this->deleteErroneousEdgeBasedOnExpectedLongReadLink(threshold, numThread);
        totalDelete += numDelete;
        cerr << "NUM_REMOVED_EDGES =" << numDelete << endl;
    } while (numDelete > 0);
