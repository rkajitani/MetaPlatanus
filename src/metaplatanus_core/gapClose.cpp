...
const unsigned short GapClose::Closer::MIN_NUM_READS_COVERING_SMALL_GAP = 1;
const int GapClose::MIN_OVERLAP_FOR_CIRCLE = 100;
const unsigned GapClose::Closer::BRUIJN_MIN_KMER = 20;
const unsigned GapClose::Closer::BRUIJN_MAX_KMER = 80;
const unsigned GapClose::Closer::MIN_COVERAGE = 2;
...
void GapClose::usage(void) const
{
    ...
    std::cerr << "\nUsage: metaplatanus_core gap_close [Options]\n"
              << "Options:\n"
...
//        sort(libraryMT.begin(), libraryMT.end());
	...
	closer.setClusterFlag(optionBool["-cluster"]);
    contig.clear();
    closer.makeGapTable();
    ...
    while (fread(&id, sizeof(long), 1, unusedFP)) {
        ...
        for (int i = 0; i < length; ++i) {
            if (tmpSeq[i] != 'N') {
                ...
                gap[id].seq.emplace_back(str.begin(), str.begin() + it);
                it = 0;
            }
        }
    }
...
{
	const string CLUSTER_PREFIX = "cluster";
    ...
    for (unsigned long long scafID = 0 ; static_cast<long>(scafID) < numScaffold; ++scafID) {
		...
		if (this->clusterFlag) {
			size_t pos = gapClosedSeq[scafID].baseName.find(CLUSTER_PREFIX);
			if (pos != std::string::npos || pos + CLUSTER_PREFIX.size() + 1 < gapClosedSeq[scafID].baseName.size())
				gapClosedSeq[scafID].clusterID = atol(gapClosedSeq[scafID].baseName.substr(pos + CLUSTER_PREFIX.size()).c_str());
		}
...
{
	vector<string> newNameBuffer(gapClosedSeq.size());
	...
	for (auto it = gapClosedSeq.begin(); it != gapClosedSeq.end(); ++it) {
		...
		if (it->baseName.size() > 0)
			oss << it->baseName.substr(0, i);
		else
			oss << "scaffold" << seqID;

		unsigned long ossputSeqLength = it->seq.size();
		if (!extentionFlag)
			ossputSeqLength -= (it->leftExtendedLen + it->rightExtendedLen);
		oss << "_len" << ossputSeqLength << "_cov" << it->coverage << "_read" << readLength << "_maxK" << contigMaxK;
		if (this->clusterFlag)
			oss << "_cluster" << it->clusterID;
		if (it->circularFlag)
			oss << "_circular_candidate";
    ...
    std::ofstream compOut(outPrefix + "_gapClosedComponent.tsv");
	for (auto it = gapClosedSeq.begin(); it != gapClosedSeq.end(); ++it) {
		if (!(it->name.empty()))
			compOut << it->name << '\t' << it->baseName << '\n';
	}
	compOut.close();
}
...
FILE *GapClose::Closer::extraAssemble(const unsigned long long numThread)
{
    FILE *temporaryFP[numThread];
    const unsigned long long size = this->gap.size();
    std::vector<GAP *> buffer(size);
	vector<vector<long> > temporaryIDBuffer(numThread);
	...
	for (long threadID = 0; threadID < numThread; ++threadID) {
		for (unsigned long long i = threadID; i < size; i += numThread) {
			if (buffer[i]->state != GAP::UNCLOSED || buffer[i]->tooManyReads)
				continue;

			GapCloseDBG<BRUIJN_MIN_KMER, BRUIJN_MAX_KMER> ele(BRUIJN_MIN_KMER, BRUIJN_MAX_KMER, MIN_COVERAGE, minOverlapDe, maxMissRate, buffer[i]);

			ele.gapAssemble();
			long numSeq = ele.saveContig(temporaryFP[threadID]);
			if (this->clusterFlag) {
				long preSize = temporaryIDBuffer[threadID].size();
				temporaryIDBuffer[threadID].resize(preSize + numSeq);
				std::fill(temporaryIDBuffer[threadID].begin() + preSize, temporaryIDBuffer[threadID].end(), buffer[i]->scaffoldID);
			}
		}
	}

	if (this->clusterFlag) {
		this->extraScaffoldID = temporaryIDBuffer[0];
		temporaryIDBuffer[0].clear();
	}
    ...
    for (unsigned long long i = 1; i < numThread; ++i) {
		...
		if (this->clusterFlag) {
			std::copy(temporaryIDBuffer[i].begin(), temporaryIDBuffer[i].end(), std::back_inserter(this->extraScaffoldID));
			temporaryIDBuffer[i].clear();
		}
    }
...
void GapClose::Closer::printExtraContig(const string &outPrefix, FILE *contigFP, const double averageReadLength, const unsigned long long readLength, const unsigned long long contigMaxK)
{
	const string seqNamePrefix = "added_seq";
	const double coverageRatio = static_cast<double>(averageReadLength) / (averageReadLength - BRUIJN_MAX_KMER + 1.0);

	std::ofstream out(outPrefix + "_extraContig.fa");

	vector<std::pair<string, long> > nameInfo;
	...
	while (fread(&length, sizeof(unsigned long long), 1, contigFP)) {
		...
		++numSeq;
		std::ostringstream oss;
		oss << seqNamePrefix << numSeq << "_len" << length << "_cov" << static_cast<unsigned short>(cov * coverageRatio + 0.5) << "_read" << averageReadLength << "_maxK" << contigMaxK;
		if (this->clusterFlag)
			oss << "_cluster" << gapClosedSeq[abs(extraScaffoldID[contigIndex]) - 1].clusterID;

		out << ">" << oss.str() << "\n";
		if (this->clusterFlag)
			nameInfo.push_back(std::make_pair(oss.str(), extraScaffoldID[contigIndex]));
    ...
    std::ofstream originOut(outPrefix + "_extraContigOrigin.tsv");
	for (auto it = nameInfo.begin(); it != nameInfo.end(); ++it) {
		originOut << it->first << '\t' << gapClosedSeq[abs(it->second) - 1].baseName << '\n';
	}
	originOut.close();
}
