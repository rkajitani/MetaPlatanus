...
namespace platanus
{
    ...
    enum ERROR
    {IO=0, FOPEN, TMP, FORMAT, READ, KMERDIST, MAP, LINK, BASE, COVERAGE, DOUBLEHASH, CLINK, CDIR, SCAF, SOLVE, GAP, DIV, COV_TRIM, MERGE, CLUSTER, MGA};
...
// set constatnt parameter 
    struct ConstParam
    {
        ...
        static const double DEFAULT_EMP_DISTR_BIN_SIZE;
    };
//////////////////////////////////////////////////////////////////////////////////////
...
// throwing object in exception
    ...
    struct LinkError : public ErrorBase
    {
        LinkError() = delete;
        LinkError(const std::string &mes): ErrorBase(LINK, mes) {}
        ~LinkError() {}
    };
    ...
    struct CovTrimError : public ErrorBase
    {
        CovTrimError(): ErrorBase(COV_TRIM, "cov_trim exception!!\nmeta_platanus cov_trim cannot finished correctly") {}
        ~CovTrimError() {}
    };
    ...
    struct ClusterError : public ErrorBase
    {
        ClusterError(): ErrorBase(CLUSTER, "Cluster exception!!\nplatanus cluster cannot finished correctly") {}
        ~ClusterError() {}
    };

    struct MgaError : public ErrorBase
    {
        MgaError(): ErrorBase(MGA, "MGA exception!!\nmga cannot be executed successfuly.") {}
        ~MgaError() {}
    };
//////////////////////////////////////////////////////////////////////////////////////
    ...
    inline bool withinRatioThreshold(const long val1, const long val2, double ratio)
    {
        return (val1 * ratio >= val2) && (val2 * ratio >= val1);
    }

    inline void printContig(const std::string &outputFilename, FILE *contigFP, const double coverageRatio, const double averageLength, const long maxKmerLength, const char *seqNamePrefix)
    {
...
// this class has read information from file.
	...
	template <typename T>
	unsigned long getNonNLength(const T &seq)
	{
		long n = 0;;
	...
	template <typename T>
	T vectorMedian(const std::vector<T> &vec)
	{
...
// this class has contig information from contig file.
    struct Contig
    {
        ...
        void execMga(const string &inFilename, const string &outFilename, const string &mgaExecutable);
        void readMgaResult(const string &mgaOutFilename);
	...
	struct LongReadAlignment
	{
		...
		LongReadAlignment(): readStart(0), readEnd(0), targetPosition(0, 0) {}
		~LongReadAlignment() {}
	};
	...
	public:
        ...
        empiricalDistribution(): binSize(ConstParam::DEFAULT_EMP_DISTR_BIN_SIZE), histo(0), distribution(0) {}
        empiricalDistribution(const double b): binSize(b), histo(0), distribution(0) {}
        ~empiricalDistribution() {}

		void count(const double value);
		void setDistribution();
		double probGreater(const double value);
	};
