...
{
    std::cerr << "\nUsage metaplatanus_core assemble [Options]\n"
              << "Options:\n"
...
void Assemble::initialKmerAssemble(vector<unsigned> &k, FILE **readFP, FILE **allReadFP, const unsigned long long numThread, BruijnGraph<KMER> &graph, Counter<KMER> &counter)
{
    ...
    for (unsigned long long i = 0; i < numThread; ++i) {
        graph.extractReadLinkingDeadEnds(readFP[i], &extractFP[i]);
    }

    DoubleHash<typename KMER::keyType, unsigned short> tmpOccurrenceTable(this->doubleHashSize);
    graph.template saveContig<KMER>(k[0], 1.0, this->lowerCoverageCutoff / 2, tmpOccurrenceTable);
    graph.deleteAllTable();
    counter.swapOccurrenceTable(k[0], tmpOccurrenceTable);

    counter.makeKmerReadDistributionConsideringPreviousGraph(k[0], extractFP, memory, numThread);
    for (unsigned long long i = 0; i < numThread; ++i) {
        fclose(extractFP[i]);
    }
...
//    lowKmerGraph.divideStraightNode(allReadFP, coverageCutoff[position - 1], this->doubleHashSize, numThread);
    ...
    if (k <= 32) {
        ...
        lowKmerGraph.template saveContig<Kmer31>(k, 1.0, this->upperCoverageCutoff, tmpOccurrenceTable);
        lowKmerGraph.deleteAllTable();
        kmer31Counter.swapOccurrenceTable(k, tmpOccurrenceTable);
        redoAssemble(k, readFP, allReadFP, numThread, kmer31Graph, kmer31Counter, position);
    } else if (k <= 64) {
        ...
        lowKmerGraph.template saveContig<KmerN<Binstr63> >(k, 1.0, this->upperCoverageCutoff, tmpOccurrenceTable);
        lowKmerGraph.deleteAllTable();
        kmer63Counter.swapOccurrenceTable(k, tmpOccurrenceTable);
        redoAssemble(k, readFP, allReadFP, numThread, kmer63Graph, kmer63Counter, position);
    } else if (k <= 96) {
        ...
        lowKmerGraph.template saveContig<KmerN<Binstr95> >(k, 1.0, this->upperCoverageCutoff, tmpOccurrenceTable);
        lowKmerGraph.deleteAllTable();
        kmer95Counter.swapOccurrenceTable(k, tmpOccurrenceTable);
        redoAssemble(k, readFP, allReadFP, numThread, kmer95Graph, kmer95Counter, position);
    } else if (k <= 128) {
        ...
        lowKmerGraph.template saveContig<KmerN<Binstr127> >(k, 1.0, this->upperCoverageCutoff, tmpOccurrenceTable);
        lowKmerGraph.deleteAllTable();
        kmer127Counter.swapOccurrenceTable(k, tmpOccurrenceTable);
        redoAssemble(k, readFP, allReadFP, numThread, kmer127Graph, kmer127Counter, position);
    } else if (k <= 160) {
        ...
        lowKmerGraph.template saveContig<KmerN<Binstr159> >(k, 1.0, this->upperCoverageCutoff, tmpOccurrenceTable);
        lowKmerGraph.deleteAllTable();
        kmer159Counter.swapOccurrenceTable(k, tmpOccurrenceTable);
        redoAssemble(k, readFP, allReadFP, numThread, kmer159Graph, kmer159Counter, position);
    } else {
        ...
        lowKmerGraph.template saveContig<KmerN<binstr_t> >(k, 1.0, this->upperCoverageCutoff, tmpOccurrenceTable);
        lowKmerGraph.deleteAllTable();
        kmerNCounter.swapOccurrenceTable(k, tmpOccurrenceTable);
        redoAssemble(k, readFP, allReadFP, numThread, kmerNGraph, kmerNCounter, position);
    }
}
...
void Assemble::saveGraph(const unsigned k, FILE **readFP, const unsigned long long numThread, BruijnGraph<KMER> &graph, Counter<KMER> &counter)
{
    ...
    graph.saveAllKmer(counter);
    cerr << "extracting read regions matching kmers in a graph..." << endl;
    # pragma omp parallel for schedule(static, 1)
    for (i = 0; i < numThread; ++i)
        counter.pickupMatchedRegionInRead(&readFP[i]);
...
//////////////////////////////////////////////////////////////////////////////////////
template <typename KMER>
void Assemble::redoAssemble(const unsigned k, FILE **readFP, FILE **allReadFP, unsigned long long numThread, BruijnGraph<KMER> &graph, Counter<KMER> &counter, const unsigned long long position)
{
    FILE *extractFP[numThread];
    counter.makeKmerReadDistributionConsideringPreviousGraph(k, readFP, memory, numThread);
    cerr << "COVERAGE_CUTOFF = " << this->lowerCoverageCutoff << endl;


    // load kmer from temporary file
    FILE *sortedKeyFP = counter.sortedKeyFromKmerFile(this->lowerCoverageCutoff);
    this->doubleHashSize = counter.loadKmer(this->lowerCoverageCutoff, this->doubleHashSize);
...
void Assemble::outputAndAfterTreatment(const unsigned k, const unsigned initialK, FILE **readFP, BruijnGraph<KMER> &graph, const unsigned long long numThread)
{
    ...
    graph.crushBubbleIterative();
    graph.deleteErroneousStraightNodeIterative(k, this->upperCoverageCutoff, numThread);
    graph.crushBubbleIterative();
    for (unsigned long long i = 0; i < numThread; ++i)
        fclose(readFP[i]);
    ...
    platanus::printContig(outputFilename, junctionFP, 1.0, averageLength, k, "junction");
	fclose(junctionFP);
}
