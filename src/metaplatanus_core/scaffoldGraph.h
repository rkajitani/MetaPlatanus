...
protected:
    ...
    struct GraphNode
    {
		...
		long clusterID;
		double coverage;
		double longReadCoverage;

        GraphNode(): isHomo(false), state(0), length(0), numEdge(0), edge(), numContig(0), contig(), numMappedTag(), oppositeBubbleNodeID(0), clusterID(0), coverage(0.0), longReadCoverage(0.0) {}
    ...
    struct GraphLayout
    {
        ...
        GraphLayout(): id(0), start(0), end(0), distance(0), numLink(0), score(0) {}
        ~GraphLayout() = default;
...
//    void setTargetLibraryIndex(const long index) { targetLibraryIndex = index; averageCoverage = (*allLibraryMT)[index][0].getAverageCoverage(); }
	...
	void setMinTagReadToLoad(const long num) { minTagReadToLoad = num; }
	...
	long joinUnambiguousNodePairGapped(const long numThread);
