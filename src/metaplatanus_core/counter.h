...
public:
    ...
    Counter(const unsigned long long kmer): lengthDistribution(), occurrenceDistribution(), occurrenceTable(), occurrenceGapTable(), kmerLength(kmer), maxOccurrence(0), kmerFP(NULL){}
    ~Counter() {}
    ...
    unsigned short findValue(const typename KMER::keyType &key) const
    {
    ...
    void pickupReadMatchedEdgeKmer(FILE **readFP);
    void pickupMatchedRegionInRead(FILE **readFP);
	void pickupReadMatchedDeadEndKmerPair(FILE **allReadFP, FILE **newReadFP);
    FILE *sortedKeyFromKmerFile(const unsigned long long minOccurrence);
    void outputOccurrenceTableBinary(const std::string &filename);
    void readOccurrenceTableBinary(const std::string &filename);
    void reCalculateCoverage(FILE **contigFP);
	...
	void getCoverageDiffRateDistributionFromContig(const std::vector<platanus::SEQ> &contig, platanus::empiricalDistribution &empDistr, const u64_t minLength);
...
unsigned long long Counter<KMER>::makeKmerReadDistributionMT(const unsigned long long  kLength, FILE **readFP, const unsigned long long memory, const unsigned long long numThread)
{
    ...
    occurrenceDistribution.clear();
    occurrenceDistribution.resize(UINT16_MAX + 1);
        ...
        for (unsigned i = 0; i < seq.length - kmerLength + 1; ++i) {
            ...
            */
            ...
            if (ptr < std::max(static_cast<unsigned long long >(contig.coverage[contigID]), minOccurrence)) {
                ptr = std::min(std::max(static_cast<unsigned long long >(contig.coverage[contigID]), minOccurrence), static_cast<unsigned long long >(UINT16_MAX - 1));
            }
        }
    }
    occurrenceDistribution.clear();
...
//////////////////////////////////////////////////////////////////////////////////////
template <typename KMER>
void Counter<KMER>::pickupReadMatchedEdgeKmer(FILE **readFP)
{
...
/*
for (auto itr = seq.base.begin(); itr != seq.base.end(); ++itr)
	std::cout << (int)(*itr);
std::cout << "\n";
*/
...
template <typename KMER>
void Counter<KMER>::pickupMatchedRegionInRead(FILE **readFP)
{
    platanus::SEQ seq, tmpSeq;
    FILE *newReadFP = platanus::makeTemporaryFile();
    rewind(*readFP);
    ...
    while (seq.readTemporaryFile(*readFP)) {
		...
		std::vector<char> matchFlag(seq.length - kmerLength + 1, 0);
        ...
        for (unsigned i = 0; i < matchFlag.size(); ++i) {
			if (!matchFlag[i])
				continue;

			unsigned start = i;
			for (; i < matchFlag.size(); ++i) {
				if (!matchFlag[i])
					break;
			}
			unsigned end = i + kmerLength - 1;
...
/*
// std::cout << start << "\t" << end << "\n";
for (auto itr = tmpSeq.base.begin(); itr != tmpSeq.base.end(); ++itr)
	std::cout << (int)(*itr);
std::cout << "\t";
*/
			tmpSeq.writeTemporaryFile(newReadFP);
		}
// std::cout << "\n";
    }
//	fclose(*readFP);
...
template <typename KMER>
void Counter<KMER>::pickupReadMatchedDeadEndKmerPair(FILE **allReadFP, FILE **newReadFP)
{
    platanus::SEQ seq, tmpSeq;
    *newReadFP = platanus::makeTemporaryFile();
    rewind(*allReadFP);
    KMER key(kmerLength);
    const unsigned long long mask = kmerLength >= 32 ? ~static_cast<unsigned long long>(0) : ~(~static_cast<unsigned long long>(0) << (2 * kmerLength));

    while (seq.readTemporaryFile(*allReadFP)) {
        if (static_cast<unsigned>(seq.length) < kmerLength)
			continue;

		unsigned start = tmpSeq.length;
		unsigned end = 0;
...
FILE *Counter<KMER>::sortedKeyFromKmerFile(const unsigned long long minOccurrence)
{
    ...
    while(kmer.readTemporaryFileForward(kmerFP)) {
        fread(&occurrence, sizeof(unsigned short), 1, kmerFP);
        if (occurrence < minOccurrence || occurrence == UINT16_MAX)
			continue;
...
template <typename KMER>
void Counter<KMER>::outputOccurrenceTableBinary(const std::string &filename)
{
    std::ofstream ofs(filename.c_str(), std::ios::binary);
    if (!ofs) throw platanus::FILEError(filename);

    ofs.write(reinterpret_cast<char *>(&(this->kmerLength)), sizeof(unsigned long long ));
    occurrenceTable.writeTable(ofs);
    ofs.close();
}


template <typename KMER>
void Counter<KMER>::readOccurrenceTableBinary(const std::string &filename)
{
    std::ifstream ifs(filename.c_str(), std::ios::binary);
    if (!ifs) throw platanus::FILEError(filename);

    ifs.read(reinterpret_cast<char *>(&(this->kmerLength)), sizeof(unsigned long long));
    occurrenceTable.readTable(ifs);
    ifs.close();
}


template <typename KMER>
void Counter<KMER>::reCalculateCoverage(FILE **contigFP)
{
    unsigned long long length;
    unsigned short coverage;
    const unsigned long long mask = kmerLength >= 32 ? ~static_cast<unsigned long long>(0) : ~(~static_cast<unsigned long long>(0) << (2 * kmerLength));

    std::cerr << "K = " << this->kmerLength << ", re-calculating coverage..." << std::endl;
    rewind(*contigFP);
    KMER kmer(this->kmerLength);
    FILE *outFP = platanus::makeTemporaryFile();

    while (fread(&length, sizeof(unsigned long long), 1, *contigFP)) {
        fread(&coverage, sizeof(unsigned short), 1, *contigFP);
        binstr_t seq(length);
        seq.readTemporaryFile(*contigFP);

        if (length < this->kmerLength) continue;

        long sum = 0;

        for (unsigned long long i = 0; i < this->kmerLength - 1; ++i) {
            kmer.setForward(this->kmerLength - 2 - i, seq.get(length - 1 - i));
            kmer.setReverse(i + 1, 0x3 ^ seq.get(length - 1- i));
        }
        for (unsigned long long i = this->kmerLength - 1; i < length; ++i) {
            kmer.forward <<= 2;
            kmer.maskForward(mask);
            kmer.setForward(0, seq.get(length - 1 - i));
            kmer.reverse >>= 2;
            kmer.setReverse(this->kmerLength - 1, 0x3 ^ seq.get(length - 1 - i));
            sum += std::max(occurrenceTable.find_any(std::min(kmer.forward, kmer.reverse))->second, static_cast<unsigned short>(1));
        }
        unsigned short cov = static_cast<unsigned short>(static_cast<double>(sum) / (length - this->kmerLength + 1) + 0.5);

        fwrite(&length, sizeof(unsigned long long), 1, outFP);
        fwrite(&cov, sizeof(unsigned short), 1, outFP);
        seq.writeTemporaryFile(outFP);
...
template <typename KMER>
void Counter<KMER>::getCoverageDiffRateDistributionFromContig(const std::vector<platanus::SEQ> &contig, platanus::empiricalDistribution &empDistr, const u64_t minLength)
{
    const unsigned long long mask = kmerLength >= 32 ? ~static_cast<unsigned long long>(0) : ~(~static_cast<unsigned long long>(0) << (2 * kmerLength));
    KMER kmer(kmerLength);

    for (unsigned long long i = 0; i < contig.size(); ++i) {
        const platanus::SEQ &seq = contig[i];
        if (static_cast<unsigned long long>(seq.length) < 2 * std::max(minLength, kmerLength)) continue;
        unsigned long long numKmer = seq.length - kmerLength + 1;

        unsigned long long leftSum = 0;
        unsigned long long leftNum = 0;
        unsigned long long rightSum = 0;
        unsigned long long rightNum = 0;
        ...
        while (start < numKmer) {
			...
			unsigned long long coverage = std::max(findValue(std::min(kmer.forward, kmer.reverse)), static_cast<unsigned short>(1));

			if (start <= static_cast<unsigned long long>(seq.length)/2 - kmerLength) {
				leftSum += coverage;
				++leftNum;
			}
			else if (start >= static_cast<unsigned long long>(seq.length)/2) {
				rightSum += coverage;
				++rightNum;
			}
		...
		if (leftNum > 0 && rightNum > 0) {
			double leftMean = static_cast<double>(leftSum) / leftNum;
			double rightMean = static_cast<double>(rightSum) / rightNum;
			empDistr.count(leftMean > rightMean ? leftMean/rightMean : rightMean/leftMean);
		}
    }
