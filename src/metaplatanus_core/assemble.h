...
private:
    // constant parameter
	static const unsigned MIN_KMER_LENGTH;
    static const unsigned SMOOTHING_WINDOW;
    static const double MAX_COVERAGE_CUT_DIFF_RATE;


    unsigned long long numExtension;
    double averageLength;
    double averageCoverage;
    double minKmerLengthRatio;
    double maxKmerLengthRatio;
    unsigned long long numThread;
    unsigned long long upperCoverageCutoff;
    unsigned long long lowerCoverageCutoff;
    ...
    std::string tableBinaryName;



    void initializeParameters(void);
    template <typename KMER> void initialKmerAssemble(std::vector<unsigned> &k, FILE **readFP, FILE **allReadFP, const unsigned long long numThread, BruijnGraph<KMER> &graph, Counter<KMER> &counter);
    ...
    template <typename KMER> void saveGraph(const unsigned k, FILE **readFP, const unsigned long long numThread, BruijnGraph<KMER> &graph, Counter<KMER> &counter);
    template <typename KMER> void redoAssemble(const unsigned k, FILE **readFP, FILE **allReadFP, unsigned long long numThread, BruijnGraph<KMER> &graph, Counter<KMER> &counter, const unsigned long long position);
    template <typename KMER> void outputAndAfterTreatment(const unsigned k, const unsigned initialK, FILE **readFP, BruijnGraph<KMER> &graph, const unsigned long long numThread);
    ...
    void extendKmer(FILE **readFP, std::vector<unsigned> &kmer);
    double getAverageLengthFromReadFile(FILE **readFP);
